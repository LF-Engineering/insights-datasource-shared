...
resource "aws_s3_bucket" "terraform-state" {
  ...
  tags = {
    Name        = "Insights V2 terraform state Dev"
    Environment = "dev"
  }
}
  ...
  container_definitions    = jsonencode([
    {
      ...
      image     = "${var.eg_account_id}.dkr.ecr.${var.eg_aws_region}.amazonaws.com/insights-connector-githubstats:latest"
      cpu       = 128
      memory    = 512
      essential = true
      logConfiguration : {
...
resource "aws_ecs_task_definition" "insights-connector-git-orphaned-handler-task" {
  ...
  container_definitions    = jsonencode([
    {
      ...
      image     = "${var.eg_account_id}.dkr.ecr.${var.eg_aws_region}.amazonaws.com/insights-connector-git-orphaned-handler:latest"
      cpu       = 1024
      memory    = 4096
      essential = true
      secrets : [
